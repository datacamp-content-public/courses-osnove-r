...
title: 'Vektori (Vectors)'
description: 'U ovom poglavlju vodimo vas na put u Vegas, gde ćete naučiti kako da analizirate rezultate kockanja korišćenjem vektora u R! Nakon završetka ovog poglavlja moći ćete da kreirate vektore u R, da im dajete nazive, da izdvojite njihove elemente i da poredite različite vektore.'
---
...
Da li osećate da imate sreće? Pripremite se, ovo poglavlje vas vodi na put u Grad grehova poznat kao "Raj za statističare"

Zahvaljujući R i vašim novim analitičkim sposobnostima, naučićete kako da unapredite svoje performanse nad tabelama i započnete karijeru kao profesionalni kockar. U ovom poglavlju naučićete kako da pratite svoj napredak u kockanju i kako da uradite jednostavne analize prethodnih koraka. Sledeća stanica, Vegas Baby... VEGAS!!

`@instructions`
- Da li se sećate šta ste naučili Do you still remember what you have learned in the first chapter? Assign the value `"Go!"` to the variable `vegas`. Remember: R is case sensitive!

`@hint`
Otkucajte sledeći kod u editor:
```
vegas <- "Go!"
```
...
`@sct`
```{r}
test_object("vegas", incorrect_msg = "Da li ste dodelili dobru vrednost promenljivoj `vegas`? R razlikuje velika i mala slova!")
success_msg("Odlično! Pređite na sledeću vežbu.")
```
